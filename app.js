import { controlModel, createState, focusSwipeEvent, handle, keyboardEvent, model, paginatePrayerByFit, parseBundle, parseCollects, prayerAvailableHeight, screenClickDecision, screenHtml, stateAfterDateChange, stateForDate, swipeEvent, upcomingFeastDays } from "./bookmark-engine.js?v=0.3.58";
import { bindFeastLinksPreference, initializeFeastLinks } from "./feast-link-preference.js?v=0.3.58";
import { calendarEventIconAssetPath, renderPixelArtStack } from "./pixel-art.js?v=0.3.58";
import { bindPsalmPreference, createPsalmBoundaryTimer, initializePsalmPreference, psalmOfficeAt, refreshPsalmDisplay } from "./psalm-preference.js?v=0.3.58";
import { initializeTheme, setThemeMode, syncSystemTheme } from "./theme.js?v=0.3.58";
import { appVersionLabel } from "./version.js?v=0.3.58";

const APP_ROOT = new URL(".", window.location.href);
const CONTENT_ROOT = APP_ROOT.pathname.endsWith("/web/") ? new URL("../", APP_ROOT) : APP_ROOT;
const PACK_URL = new URL("firmware/circuitpython/readings.active.jsonl?v=0.3.58", CONTENT_ROOT);
